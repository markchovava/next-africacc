...
export default function PartnerEdit({ id }) {
  const router = useRouter();
  const [data, setData] = useState();
  const [image, setImage] = useState();
  const [roles, setPartners] = useState();
  ...
  const postData = async () => {
    ...
    try{
        const result = await axiosClientAPI.post(`partner/${id}`, formData, config)
        .then((response) => {
        ...
        <div className='w-[100%] grid grid-cols-2 gap-6 mb-4'>
          ...
          <div className='w-[100%]'>
            ...
            <select 
              ...
              {[...Array(12)].map((i, key) => (
                <option value={key+1} selected={data?.priority == key+1 && 'selected'}>
                  {key+1}</option>
              ))}
            </select>
          </div>
