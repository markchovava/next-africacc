...
export default function PartnerList() {
  ...
  async function getData() {
    try{
      const result = await axiosClientAPI.get(`partner`, config)
      .then((response) => {
        setData(response.data.data)
        setMeta(response.data.meta)
        setPrevURL(response.data.links.prev)
  ...
  async function searchData() {
    ...
    try{
      const result = await axiosClientAPI.get(`partner?search=${search}`, config)
      .then((response) => {
        setData(response.data.data)
        setMeta(response.data.meta)
        setPrevURL(response.data.links.prev)
  ...
  async function deleteData(id) {
    try{
      const result = await axiosClientAPI.delete(`partner/${id}`, config)
      .then((response) => {
        ...
        <section className="w-[100%] lg:overflow-hidden overflow-scroll">
          ...
          <section className='lg:w-[100%] w-[70rem]'>
            <div className='w-[90%] text-lg py-2 mx-auto flex items-center justify-start font-bold font-white bg-slate-200 '>
              ...
              <div className='w-[35%] border-r border-white px-3 py-2'>LINK</div>
            ...
            { data.length > 0 ?
              data.map((i, key) => (
                <div key={key} className='w-[90%] text-lg border-x border-b border-slate-300 mx-auto flex items-center justify-start '>
                  ...
                  <div className='w-[35%] border-r border-blue-300 px-3 py-2'>
                    {i?.link}
                  </div>
                  ...
                  <div className='w-[15%] px-3 py-2 text-xl flex items-center justify-start gap-4'>
                    <Link title='View partner' href={`/admin/partner/${i.id}`}> 
                      <FaEye className='hover:text-blue-500 duration-150 hover:scale-110 transition-all ease-in'/> 
                    </Link>
                    <Link title='Edit partner' href={`/admin/partner/edit/${i.id}`}> 
