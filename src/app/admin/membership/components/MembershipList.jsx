...
export default function MembershipList() {
  ...
  async function getData() {
    try{
      const result = await axiosClientAPI.get(`membership`, config)
      .then((response) => {
        setData(response.data.data)
        setMeta(response.data.meta)
        setPrevURL(response.data.links.prev)
  ...
  async function searchData() {
    ...
    try{
      const result = await axiosClientAPI.get(`membership?search=${search}`, config)
      .then((response) => {
        setData(response.data.data)
        setMeta(response.data.meta)
        setPrevURL(response.data.links.prev)
            ...
            { data.length > 0 ?
              data.map((i, key) => (
                <div key={key} className='w-[90%] text-lg border-x border-b border-slate-300 mx-auto flex items-center justify-start '>
                  <div className='w-[40%] border-r border-slate-300 px-3 py-2 flex items-center justify-between'>
                  ...
                  <div className='w-[15%] px-3 py-2 flex items-center justify-start gap-4'>
                    <Link title='View membership' href={`/admin/membership/${i.id}`}> 
                      <FaEye className='text-xl hover:text-blue-500 duration-150 hover:scale-110 transition-all ease-in'/> 
                    </Link>
                    <Link title='Edit membership' href={`/admin/membership/edit/${i.id}`}> 
