...
export default function CountryAdd({ id }) {
  ...
  const postData = async () => {
    ...
    try{
        const res = await axiosClientAPI.post(`country`, formData, config)
        startTransition(() => res)
        if(res.data?.status == 1){
          toast.success(res.data?.message, toastifyDarkBounce);
        ...
        <form action={postData}>
          ...
          <div className='flex items-center justify-center py-[1rem]'>
            <button onClick={() => setIsSubmit(true)} className='btn__primary'>
              {isPending == true ? 'Processing' : 'Submit'}
            </button>
          </div>
        </form>
      </div>
