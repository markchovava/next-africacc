...
export default function TestimonialList() {
  ...
  async function getData() {
    try{
      const result = await axiosClientAPI.get(`testimonial`, config)
      .then((response) => {
        setData(response.data.data)
        setMeta(response.data.meta)
        setPrevURL(response.data.links.prev)
  ...
  async function searchData() {
    ...
    try{
      const result = await axiosClientAPI.get(`testimonial?search=${search}`, config)
      .then((response) => {
        setData(response.data.data)
        setMeta(response.data.meta)
        setPrevURL(response.data.links.prev)
  ...
  async function deleteData(id) {
    try{
      const result = await axiosClientAPI.delete(`testimonial/${id}`, config)
      .then((response) => {
    ...
    <>
     <section className='w-[100%]'>
          <div className='w-[90%] mx-auto flex items-center justify-between pb-[1rem]'>
            <div className='md:w-[45%] w-[75%] flex items-center justify-start'>
              <input 
                type='text' 
                value={search}
            ...
            { data.length > 0 ?
              data.map((i, key) => (
                <div key={key} className='w-[90%] text-lg border-x border-b border-slate-300 mx-auto flex items-center justify-start '>
                  <div className='w-[35%] border-r border-slate-300 px-3 py-2 flex items-center justify-between'>
                    <span className=''>{i.name}</span>
                    <span className='bg-green-700 px-2 rounded-md text-white text-center'>
                      {i.priority}</span>
                  </div>
                  ...
                  <div className='w-[15%] px-3 py-2 flex items-center justify-start gap-4'>
                    <Link title='View testimonial' href={`/admin/testimonial/${i.id}`}> 
                      <FaEye className='text-xl hover:text-blue-500 duration-150 hover:scale-110 transition-all ease-in'/> 
                    </Link>
                    <Link title='Edit testimonial' href={`/admin/testimonial/edit/${i.id}`}> 
                      <MdEdit className='text-xl hover:text-green-500 duration-150 hover:scale-110 transition-all ease-in' /> 
                    </Link>  
                    <button> 
                        <MdDeleteForever 
                          onClick={() => deleteData(i.id)}
                          className='text-xl hover:text-red-500 duration-150 hover:scale-110 transition-all ease-in' /> 
                    </button>
                  </div>
                </div>
              ))
