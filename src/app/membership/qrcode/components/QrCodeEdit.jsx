...
export default function QrCodeEdit({ dbData }) {
    const router = useRouter();
    const { getMembershipToken, setMembershipToken } = tokenMembership()
    ...
    const postData = async () => {
        ...
        try{
            const result = await axiosClientAPI.post(`member-order-qrcode`, formData, config)
            .then((response) => {
                if(response.data.status == 1){
                  const res = response.data;
                  setMOrderToken(response?.data?.member_order);
                  setMembershipToken(res.membership_id)
