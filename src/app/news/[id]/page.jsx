...
import { getNew, getNewsList } from '@/api/getNews';
...
export default async function page({ params: {id} }) {
  const [newData, newsListData] = await Promise.all([getNew(id), getNewsList()]);
  ...
  return (
    ...
    <section 
        ...
        <div className='w-[100%] h-[100%] lg:text-[5rem] text-[3.8rem] text-center leading-none font-medium flex items-center justify-center text-white drop-shadow-md'>
          {trimString(newData?.data?.title, 30)}
        </div>
    </section>
    ...
    <section className='w-[100%] '>
        <ul className='mx-auto py-1 w-[90%] border-y border-slate-300 flex items-center justify-start gap-1'>
            ...
            <li><MdOutlineChevronRight /></li>
            <li><Link href={`/news/${id}`} className='font-semibold'>{trimString(newData?.data?.title, 50)}</Link></li>
        </ul>
    </section>

    <NewsView dbData={newData} />
