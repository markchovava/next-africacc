...
export default function CountryView({ slug, dbData, sectorData, countryData }) {
  ...
  async function paginationHandler(url) {
   ...
   async function getData() {
    try{
      const result = await axios.get(`${baseURL}country-opportunity?slug=${slug}`)
      .then((response) => {
        setData(response.data.data)
        setPrevURL(response.data.links.prev)
        setNextURL(response.data.links.next)
  ...
  async function searchData() {
    ...
    try{
      const result = await axios.get(`${baseURL}country-opportunity?slug=${slug}&search=${search}`)
      .then((response) => {
        setData(response.data.data)
        setPrevURL(response.data.links.prev)
        setNextURL(response.data.links.next)
  ...
  return (
    ...
    <section className='w-[100%] pt-[4rem] pb-[8rem]'>
        <div className='mx-auto w-[90%]'>
          {countryData?.data?.invest_link &&
          <section className='pb-[2rem]'>
            <Link href={countryData?.data?.invest_link} target='_blank'
              className='text-green-600 underline italic hover:no-underline'>
              <span>{countryData?.data?.name + ' '}</span>
                ...
                <ul className='ml-2'>
                  { sectors.length > 0 &&
                    ...
                    <li key={key} className='group mb-2'>
                      <Link 
                        href={`/country/sector/${slug}/${i.id}`} 
                        className='flex items-center justify-start gap-2 group-hover:translate-x-1 transition-all ease-in-out'>
                    ...
                    <div key={key} className='px-5 py-6 mb-6 flex md:flex-row flex-col items-center justify-start gap-8 transition-all ease-in-out bg-white drop-shadow-md hover:drop-shadow-lg'>
                      ...
                      <div className='w-[100%] lg:w-[75%] flex md:flex-row flex-col items-center justify-start gap-4'>
                        ...
                        <div className='w-[100%] lg:w-[80%]'>
                          ...
                          <p className='font-light leading-tight text-lg mb-1'>
